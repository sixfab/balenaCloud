...
class rockBlock(object):
    ...
    def ping(self):
        ...
        if( self.s.readline().strip().decode("utf-8") == command ):
            
            if( self.s.readline().strip().decode("utf-8") == "OK" ):
    ...
    def requestSignalStrength(self):
        ...
        if( self.s.readline().strip().decode("utf-8") == command):
            ...
            response = self.s.readline().strip().decode("utf-8")
                  
            if( response.find("+CSQ") >= 0 ):
                            
                self.s.readline().strip().decode("utf-8")    #OK
                self.s.readline().strip().decode("utf-8")    #BLANK
    ...
    def setup(self):
        ...
        if(self.s.readline().strip().decode("utf-8") == command and self.s.readline().strip().decode("utf-8") == "OK"):
    ...
    def _queueMessage(self, msg):
        ...
        if(self.s.readline().strip().decode("utf-8") == command):
            ...
            if(self.s.readline().strip().decode("utf-8") == "READY"):
                ...
                self.s.write( str(msg).encode("utf-8") )
                
                self.s.write( chr( checksum >> 8 ).encode("utf-8") )
                self.s.write( chr( checksum & 0xFF ).encode("utf-8") )
                                       
                self.s.readline().strip().decode("utf-8")   #BLANK
                
                result = False
                                
                if(self.s.readline().strip().decode("utf-8") == "0"):
    ...
    def _processMtMessage(self, mtMsn):
        ...
        self.s.write(("AT+SBDRB\r").encode("utf-8"))
        
        response = self.s.readline().strip().decode("utf-8").replace("AT+SBDRB\r","").strip()
    ...
    def _isNetworkTimeValid(self):
        ...
        if( self.s.readline().strip().decode("utf-8") == command ):  #Echo
